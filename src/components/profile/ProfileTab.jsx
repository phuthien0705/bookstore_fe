...
const ProfileTab = () => {
    const defaultAvatar = '/static/media/user-round.27fe79b102ea6aad2f60e66cff82818d.svg';
    const defaultGen = 'Nam';
    const defaultUserName = authService.isAuthenticated() && authService.getUser()?.name;
        ...
        <Grid container spacing={2}>
            <Grid item xs={4}>
                <MainCard title="Ảnh đại diện">
                    <Stack flexDirection="column" alignItems="center" spacing={2}>
                        <Avatar alt="Cak" src={avatar.preview || avatar} sx={{ width: 150, height: 150 }} />
                        <Typography>Tải lên/Thay đổi ảnh đại diện.</Typography>
                        <Button variant="contained" component="label" startIcon={<PhotoCamera />}>
                            Tải lên
                            <input onChange={handlePreviewAvatar} hidden accept="image/*" type="file" />
                        </Button>
                    </Stack>
                </MainCard>
            </Grid>
            <Grid item xs={8}>
                <MainCard title="Chỉnh sửa thông tin chi tiết">
                    <Stack spacing={3}>
                        <TextField id="outlined-fullname" label="Họ và tên" value={fullName} onChange={handleChangeFullName} />
                        <TextField
